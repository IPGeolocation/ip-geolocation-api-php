...
use Ipgeolocation\Sdk\Model\ModelInterface;
...
class ObjectSerializer
{
     ...
     * @param string|null $type   the ipgeoapiToolsType of the data
     * @param string|null $format the format of the ipgeoapiTools type of the data
     *
    ...
    public static function sanitizeForSerialization($data, $type = null, $format = null)
    {
        ...
        if (is_object($data)) {
            ...
            if ($data instanceof ModelInterface) {
                $formats = $data::ipgeoapiFormats();
                foreach ($data::ipgeoapiTypes() as $property => $ipgeoapiType) {
                    $getter = $data::getters()[$property];
                    $value = $data->$getter();
                    if ($value !== null && !in_array($ipgeoapiType, ['\DateTime', '\SplFileObject', 'array', 'bool', 'boolean', 'byte', 'float', 'int', 'integer', 'mixed', 'number', 'object', 'string', 'void'], true)) {
                        $callable = [$ipgeoapiType, 'getAllowableEnumValues'];
                        if (is_callable($callable)) {
                            /** array $callable */
                            $allowedEnumTypes = $callable();
                            if (!in_array($value, $allowedEnumTypes, true)) {
                                $imploded = implode("', '", $allowedEnumTypes);
                                throw new \InvalidArgumentException("Invalid value for enum '$ipgeoapiType', must be one of: '$imploded'");
                            }
                        }
                    }
                    if (($data::isNullable($property) && $data->isNullableSetToNull($property)) || $value !== null) {
                        $values[$data::attributeMap()[$property]] = self::sanitizeForSerialization($value, $ipgeoapiType, $formats[$property]);
                    }
                }
            } else {
                foreach($data as $property => $value) {
    ...
    /**
     * Checks if a value is empty, based on its ipgeoapi type.
     *
     * @param mixed  $value
     * @param string $ipgeoapiType
     *
     * @return bool true if $value is empty
     */
    private static function isEmptyValue($value, string $ipgeoapiType): bool
    {
     ...
     * @param string $ipgeoapiType ipgeoapiType eg. array or object
    ...
    public static function toQueryValue(
        ...
        #  - its value is set to a value that is equivalent to "empty", depending on its ipgeoapi type. For
        #    example, 0 as "int" or "boolean" is NOT an empty value.
        if (self::isEmptyValue($value, $ipgeoapiType)) {
            if ($required) {
                return ["{$paramName}" => ''];
            } else {
                return [];
        ...
        if($ipgeoapiType === "\\DateTime" && $value instanceof \DateTime) {
            return ["{$paramName}" => $value->format(self::$dateTimeFormat)];
        }

        $query = [];
        $value = (in_array($ipgeoapiType, ['object', 'array'], true)) ? (array)$value : $value;
        ...
        // https://github.com/OAI/ipgeoapi-Specification/blob/main/versions/3.1.0.md#style-values
        if ($ipgeoapiType === 'array' && $style === 'deepObject' && $explode) {
            return $value;
        }

        if ($ipgeoapiType === 'object' && ($style === 'deepObject' || $explode)) {
            return $value;
        }

        if ('boolean' === $ipgeoapiType && is_bool($value)) {
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if (method_exists($class, 'getAllowableEnumValues')) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\Ipgeolocation\Sdk\Model\\' . $data->{$discriminator};
            ...
            $instance = new $class();
            foreach ($instance::ipgeoapiTypes() as $property => $type) {
